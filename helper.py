...
def get_semantic_map(path):
    ...
    tmp = np.zeros(
        (semantic.shape[0], semantic.shape[1], dataset.palette.shape[0]),
        dtype=np.float32)
    for k in range(dataset.palette.shape[0]):
        tmp[:, :,
            k] = np.float32((semantic[:, :, 0] == dataset.palette[k, 0]) &
...
def print_semantic_map(semantic, path):
    ...
    color_image = dataset.palette[prediction.ravel()].reshape(
