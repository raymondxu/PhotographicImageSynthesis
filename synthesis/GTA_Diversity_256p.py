...
def compute_error(real, fake, label):
    return tf.reduce_mean(
        label * tf.expand_dims(
            tf.reduce_mean(tf.abs(fake - real), reduction_indices=[3]), -1),
...
if is_training:
    ...
    for epoch in range(1, 401):
        ...
        for ind in np.random.permutation(12403) + 1:
            ...
            if input_images[ind] is None:
                ...
                input_images[ind] = np.expand_dims(
                    np.float32(
                        scipy.misc.imread(
                            "data/GTA/RGB256Full/%08d.png" % ind)),
                    axis=0)
        ...
        for ind in range(12403 - 49, 12403 + 1) + range(
                100001, 100051):  #print intermediate results
            ...
            output = sess.run(
                ...
                feed_dict={
                    ...
                    np.concatenate(
                        (semantic,
                         np.expand_dims(1 - np.sum(semantic, axis=1), axis=1)),
            ...
            scipy.misc.toimage(
                ...
                cmin=0,
                cmax=255).save("result_GTA/%04d/%06d_output.jpg" % (epoch,
                                                                    ind))
...
for ind in range(100001, 106383):
    ...
    scipy.misc.toimage(
        ...
        cmax=255).save("result_GTA/final/%06d_output.png" % ind)
