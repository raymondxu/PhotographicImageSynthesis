...
G_opt = tf.train.AdamOptimizer(learning_rate=lr).minimize(
    ...
    var_list=[
        ...
        if var.name.startswith('g_1024') or var.name.startswith('g_512')
    ])  #only fine-tune the last two refinement module due to memory limitation
...
if ckpt:
    ...
    saver = tf.train.Saver(var_list=[
        var for var in tf.trainable_variables()
        if var.name.startswith('g_') and not var.name.startswith('g_1024')
    ])
...
if is_training:
    ...
    for epoch in range(1, 6):
        ...
        for ind in np.random.permutation(2975) + 1:
            ...
            if input_images[ind] is None:
                label_images[ind] = helper.get_semantic_map(
                    "data/cityscapes/Label1024Full/%08d.png" %
                    ind)  #training label
                input_images[ind] = np.expand_dims(
                    np.float32(
                        scipy.misc.imread(
                            "data/cityscapes/RGB1024Full_vivid/%08d.png" %
                            ind)),
                    axis=0
        ...
        for ind in range(100001, 100051):
            if not os.path.isfile("data/cityscapes/Label1024Full/%08d.png" %
                                  ind):  #test label
                continue
            semantic = helper.get_semantic_map(
                "data/cityscapes/Label1024Full/%08d.png" % ind)  #test label
            output = sess.run(
                generator,
                feed_dict={
                    label:
            ...
            scipy.misc.toimage(
                output[0, :, :, :], cmin=0, cmax=255).save(
                    "result_1024p/%04d/%06d_output.jpg" % (epoch, ind))
...
for ind in range(100001, 100501):
    ...
    scipy.misc.toimage(
        output[0, :, :, :], cmin=0,
        cmax=255).save("result_1024p/final/%06d_output.jpg" % ind)
